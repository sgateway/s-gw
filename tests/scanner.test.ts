...
import { sanitizeKnownSecrets, scanText } from "../src/scanner.js";
...
describe("secret detector rule pack", () => {
  ...
  it("validates JWT structure before tokenizing", async () => {
    const jwt = [
      ...
      Buffer.from(JSON.stringify({ sub: "1234567890", role: "operator" })).toString("base64url"),
      secretLike("scanner", "signature", "fixture", "value", "1234567890")
    ].join(".");
    const fake = secretLike(
      "eyJnotvalidpayloadnotjson",
      ".eyJalsonotjsonpayload",
...
describe("known-secret sanitization", () => {
  it("replaces overlapping credentials longest first", () => {
    const output = sanitizeKnownSecrets("secret-value secret", [
      { handle: "s-gw:credential:short", value: "secret" },
      { handle: "s-gw:credential:long", value: "secret-value" }
    ]);

...
If an existing public GitHub release missed npm or MCP Registry publication, run **Publish release** with `release_tag=vX.Y.Z` and `publish_npm_only=true`. This verifies the immutable tag and matching private core, rejects current high-severity audit findings without letting a newly disclosed low-severity advisory make the immutable tag unrepairable, publishes or verifies the scoped npm version through the existing OIDC environment, and then publishes the same version to the MCP Registry. It does not rebuild assets or alter the GitHub release.

...
flowchart TB
    ...
    subgraph Local["Trusted local s-gw boundary"]
      ...
      P["User approval"]
      S["Encrypted store"]
      R["Credential resolver and runner"]
      Z["Output sanitizer"]
    end
...
The local operating system account, s-gw process, operating system credential store, approved executable, and user approval decision are trusted for an approved action. Agent input, scanned content, command arguments, destinations, and command output are treated as untrusted.
